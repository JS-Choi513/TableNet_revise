...
class table_mask(Layer):
    ...
    def __init__(self):
        super().__init__()
        self.conv_7 = Conv2D(kernel_size=(1,1), filters=128, kernel_regularizer=tf.keras.regularizers.l2(0.001))
...
resnet_ = keras.applications.resnet_v2.ResNet50V2(
    include_top=False,
    weights="imagenet",
    input_tensor=input_,
    input_shape=None,
...
for layer in vgg19_.layers:
    print("fixed!")    
    layer.trainable = False
    layer._name = layer.name+str("_V")# resnet layer name collision

pool3 = vgg19_.get_layer('block3_pool').output
pool4 = vgg19_.get_layer('block4_pool').output

conv_1_1_1 = Conv2D(filters=128, kernel_size=(1, 1), activation='relu', name="block6_conv1", kernel_regularizer=tf.keras.regularizers.l2(0.002))(resnet_.output)
...
plt.savefig("tablenet_resnet50_100epoch_loss.png")
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig("tablenet_resnet50_100epoch_F1.png")



model.save('model/tablenet_resnet50_VGG19')
